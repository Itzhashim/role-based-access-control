...
- **Server-side enforcement is the only enforcement.** Every control was tested by bypassing the
  UI entirely; the browser portal contributed no security of its own.
- **Ownership checks are a separate obligation from role checks.** Eight of the fifteen attack
  scenarios pass the role check and are stopped only by the object-level check.
- **Uniform errors matter.** Returning 403 rather than 404 for objects that exist but are not
  the caller's — and returning the same 403 for objects that do not exist — removes an
  enumeration channel that would otherwise leak the shape of the database.

Known limitations of this implementation:

1. **No CSRF token.** Browser forms rely on the `SameSite=Lax` cookie attribute. That covers
   cross-site POSTs in current browsers, but a synchroniser token would be the belt-and-braces
   answer for a production deployment.
2. **The revocation list grows.** Expired `jti` rows are never pruned; a scheduled cleanup job
   would be needed at scale.
3. **Permissions are static in code.** Changing the matrix requires a deploy. A database-backed
   matrix with an administrative editor would suit an institution with evolving policies —
   at the cost of making the policy itself a target.
4. **Lockout is per account, not per source address.** A distributed attack across many accounts
   would not trip it; rate limiting at the edge belongs in front of this application.
5. **Development defaults.** `SECRET_KEY` has an insecure default and `COOKIE_SECURE` is off so
   the demo runs over plain HTTP. Both must be set for any real deployment, alongside TLS.

Natural next steps: refresh tokens with rotation, multi-factor authentication for the
administrator role, tamper-evident (hash-chained) audit records, and shipping the audit log to
an append-only store outside the application's own database.
